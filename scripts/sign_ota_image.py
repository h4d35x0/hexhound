...
# The human-readable half of the provenance, kept in lockstep with the
# OTA_KEY_PROVENANCE_DEVELOPMENT define below. This banner used to be emitted
# unconditionally, so a header holding a real release key still announced
# itself as a development key. That is the exact failure the machine-readable
# define was introduced to avoid, reproduced in the comment the define was
# meant to replace, and in the one file where a reader most needs to be told
# the truth.
DEV_KEY_BANNER = """// ── THIS IS A DEVELOPMENT KEY ─────────────────────────────────────────────
...
RELEASE_KEY_BANNER = """// ── THIS IS A RELEASE KEY ─────────────────────────────────────────────────
// Installed with --set-release-pubkey. The private half was generated outside
// this repository and has never been in it. Replacing it is a firmware change
// and a rotation, not an edit: every unit already in the field trusts the key
// it was flashed with, so a rotation only reaches units that are reflashed."""
...
def set_release_pubkey(pubkey_hex):
    ...
    header = HEADER_TEMPLATE.format(
        count=1,
        devkey=0,
        provenance_banner=RELEASE_KEY_BANNER,
        keyid=kid.hex(),
...
def gen_key(force):
    ...
    header = HEADER_TEMPLATE.format(
        count=1,
        devkey=1,
        provenance_banner=DEV_KEY_BANNER,
        keyid=kid.hex(),
        idbytes=OTA_KEYID_BYTES,
        idrow=", ".join("0x%02x" % b for b in kid),
        keyrows=_fmt_bytes(pk),
