...
# Human-readable provenance, kept in lockstep with the
# CONTENT_KEY_PROVENANCE_DEVELOPMENT define below. This banner was emitted
# unconditionally before 2026-09-22, so a header holding a real release key
# still announced itself as a development key. That is the very failure the
# machine-readable define exists to prevent, reproduced in the comment the
# define was meant to replace.
...
_CONTENT_RELEASE_BANNER = """\n// ── THIS IS A RELEASE KEY ─────────────────────────────────────────────────
// Installed from a public key whose private half was generated outside this
// repository and has never been in it. A leaked content key lets somebody
// write dialogue, not replace firmware, so it rotates on its own schedule."""
...
""" % (_CONTENT_DEV_BANNER if development else _CONTENT_RELEASE_BANNER,
