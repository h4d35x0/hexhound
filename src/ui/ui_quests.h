...
// ── HexHound - Daily Quest Screen ────────────────────────────────
//
// Renders QuestEngine's daily slots for SCREEN_QUESTS: each quest's text, its
// kind and its progress, with the completed ones reading as completed rather
// than merely sitting at a full bar.
//
// The list is BACK plus one row per active quest, exactly like ui_missions, so
// the one-button convention is unchanged: short press moves the cursor, long
// press acts on the highlighted row. On BACK that means leave; on a quest it
// means spend the day's reroll.
//
// QuestEngine::activeCount() is NOT assumed to be QUEST_MAX_ACTIVE. A content
// pack can leave this board with fewer eligible definitions, or none at all,
// and a screen that assumed four rows would read past the end of the slots or
// paint an empty box. Zero quests draws an explicit empty state.

// Loop / scroll bound only (never an array size), so a runtime expression is
// safe. Large panels (320x172) and the round panel fit 5 rows; the 160x80
// keeps 4. Matches MISSIONS_PER_PAGE in ui_missions.h.
#define QUESTS_PER_PAGE ((SCREEN_H > 100) ? 5 : 4)

class UIQuests {
public:
    static UIQuests& instance();

    void init(TFT_eSPI* tft);

    // Open the quest list (resets cursor and scroll).
    void open();
    ...
    // Long press target. Row 0 is BACK; every other row is a quest.
    bool backSelected() const { return _cursor == 0; }

    // Index into QuestEngine's slots, or -1 when BACK is highlighted. Valid
    // only while it is < QuestEngine::activeCount().
    int selectedQuest() const { return _cursor - 1; }

    // True when a long press on the current row would actually reroll: a quest
    // row, not yet complete, with budget left.
    bool canRerollSelected() const;

    // Performs that reroll and redraws. Returns false and changes nothing when
    // canRerollSelected() is false, so the caller can show a "no rerolls left"
    // note without duplicating the rules.
    bool rerollSelected(uint32_t seed);

private:
    UIQuests() = default;

    int  totalEntries() const;   // BACK + activeCount()
    void drawHeader();
    void drawFooter();
